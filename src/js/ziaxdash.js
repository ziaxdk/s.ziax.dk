...
module.config(['$stateProvider', '$urlRouterProvider', '$sceDelegateProvider', '$provide', '$httpProvider', 'GPSProvider',
  ...
  $stateProvider
    ...
    .state('new', {
      ...
      resolve: { Result: angular.noop, NewApiResult: ['$http', function($http) { return $http.get('/api/tags'); }] },
      // controller: 'NewController',
      controllerProvider: ['$stateParams', function($state) { console.log('hep', $state); return 'New' + $state.type + 'Controller'; }],
      // controllerAs: 'NewCtrl'
    })
/*    .state('new.article', {
      url: '/article',
...
module.controller('NewController', ['$scope', '$http', '$state', '$controller', 'NewApiResult', 'Result', 'PlaceService', 'DelayerFactory', 'DocumentService', 'TypeService', 'GazService', 'GPS',
  function ( $scope, $http, $state, $controller, NewApiResult, Result, PlaceService, DelayerFactory, DocumentService, TypeService, GazService, GPS ) {
    console.log('NewController ready...');
    $controller('NewLinkController', { $scope: $scope });
    var id,
        type,
        watcher;
    $scope.meta = {
